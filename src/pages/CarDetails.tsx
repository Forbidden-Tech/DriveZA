...
import { 
  ...
  Car, User, Phone, MessageSquare, Share2, Heart,
  Shield, CheckCircle, Loader2, Sparkles, ArrowRight
} from 'lucide-react';
...
export default function CarDetails() {
  ...
  const { data: car, isLoading, error } = useQuery<Car[]>({
    queryKey: ['car', carId],
    queryFn: () => base44.entities.Car.filter({ id: carId }),
    select: (data) => data[0],
    enabled: !!carId
  });
      ...
      <div className="min-h-screen flex items-center justify-center bg-[#f8f9fa]">
        <div className="text-center">
          ...
          <Link to={createPageUrl('Browse')}>
            <Button className="bg-violet-500 hover:bg-violet-600">Browse Cars</Button>
          </Link>
        </div>
      </div>
    );
      ...
      <div className="bg-[#0a0a0a] sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <Link to={createPageUrl('Browse')} className="flex items-center gap-2 text-gray-400 hover:text-white transition-colors">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {car.features && car.features.length > 0 && (
              ...
              >
                <Card className="border-0 shadow-xl rounded-3xl overflow-hidden">
                  <CardContent className="p-6">
                    <h2 className="text-xl font-bold text-gray-900 mb-6">Features & Extras</h2>
              ...
              >
                <Card className="border-0 shadow-xl rounded-3xl overflow-hidden">
                  <CardContent className="p-6">
                    ...
                    <p className="text-gray-600 whitespace-pre-wrap leading-relaxed">{car.description}</p>
                  </CardContent>
                </Card>
              </motion.div>
            )}
                ...
                <CardContent className="p-6">
                  <div className="flex items-start justify-between gap-4 mb-2">
                    <div>
                      ...
                      {car.variant && (
                        <p className="text-gray-500 text-sm mt-1">{car.variant}</p>
                      )}
                    </div>
                    {car.featured && (
                      <Badge className="bg-gradient-to-r from-emerald-500 to-teal-500 text-white border-0">
                  ...
                  <div className="flex items-center gap-2 text-gray-600 mb-6 pb-6 border-b">
                    ...
                    <span>{car.city || ''} {car.province || 'South Africa'}</span>
                  </div>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-medium text-gray-900">{car.seller_name || 'Seller'}</p>
                        <p className="text-sm text-gray-500">{car.seller_type} Seller</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    ...
                    <Dialog>
                      <DialogTrigger asChild>
                        <Button variant="outline" className="w-full h-12 border-2 border-emerald-200 text-emerald-600 hover:bg-emerald-50 font-semibold">
                      ...
                      <DialogContent className="rounded-3xl">
                        ...
                        <form onSubmit={handleEnquiry} className="space-y-4 mt-4">
                          ...
                          <Button type="submit" className="w-full h-12 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600">
                            Send Enquiry
              ...
              <div className="flex gap-3">
                {car.seller_phone && (
                  <Button 
                    ...
                    onClick={() => window.location.href = `tel:${car.seller_phone}`}
                  >
                    <Phone className="w-4 h-4 mr-2" />
                    Call
                  </Button>
                ...
                <Dialog>
                  <DialogTrigger asChild>
                    <Button variant="outline" className="flex-1 h-12 border-2 border-emerald-200 text-emerald-600">
                      <MessageSquare className="w-4 h-4 mr-2" />
                      Enquire
                    </Button>
                  </DialogTrigger>
                  <DialogContent className="rounded-3xl">
                    ...
                    <form onSubmit={handleEnquiry} className="space-y-4 mt-4">
                      ...
                      <div className="space-y-2">
                        <Label>Phone</Label>
                        <Input value={enquiryForm.phone} onChange={(e) => setEnquiryForm({...enquiryForm, phone: e.target.value})} required />
                      </div>
                      <div className="space-y-2">
                        <Label>Email</Label>
                        <Input type="email" value={enquiryForm.email} onChange={(e) => setEnquiryForm({...enquiryForm, email: e.target.value})} required />
                      </div>
                      <div className="space-y-2">
                        <Label>Message</Label>
                        <Textarea value={enquiryForm.message} onChange={(e) => setEnquiryForm({...enquiryForm, message: e.target.value})} rows={3} />
                      </div>
                      <Button type="submit" className="w-full bg-gradient-to-r from-emerald-500 to-teal-500">Send</Button>
                    </form>
                  </DialogContent>
                </Dialog>
              </div>
